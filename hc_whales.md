...
where $K_{whale}$ is the daily caloric requirement for killer whales in kcal/kg, $M_{ad.male}$ is the mass of an adult male killer whale, $M_{ad.female/sub)}$ is the mass of an adult female or sub-adult killer whale, $M_{juv}$ is the mass of a juvenile killer whale, $N_{m}$ is the number of Adult Males, $N_{f}$ is the number of Adult Females/Subadults, $N_{j}$ is the number of Juveniles, and t_{days} is the number of days present in Hood Canal.
...
where $K_{seal}$ is the caloric value of a seal in kcal/kg, $M_{seal}$ is the mass of a harbor seal and $A_{whale}$ is the caloric assimilation value for whales.
